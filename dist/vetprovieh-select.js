    ...
    static _iterateThrough(obj, attributes, depth = 0) {
        ...
        while (attributes.length > depth) {
            ...
            if (attribute.includes('[')) {
                ...
                const a = attribute.substr(0, index);
                const matchB = attribute.match(/\d+/);
                obj = obj[a][matchB ? matchB[0] : 0];
            }
            else {
                obj = obj[attribute];
            }
...
class ViewHelper {
    ...
    /**
     * Toggle Requirement for Element
     * @param {HTMLElement} element
     * @param {boolean} show
     */
    static toggleRequirement(element, show) {
        console.log(element);
    ...
    static replacePlaceholders(template, e) {
        Array.from(template.childNodes)
            ...
            .forEach((node) => {
            this.replacePlaceholdersInNodeText(node, e);
        });
        if (!template.innerHTML.includes('template')) {
            this.replacePlaceholdersInnerHTML(template, e);
        }
        Array.from(template.children)
            .filter((child) => child.tagName != 'TEMPLATE')
            .forEach((child) => {
            this.replacePlaceholders(child, e);
        });
    }
    /**
     * Format View with a attached Pipe
     * @param {string} pipeName
     * @param {string} value
     * @return {string}
     */
    static pipeUse(pipeName, value) {
        switch (pipeName) {
            case '|date':
                return ObjectHelper.formatDate(value);
            default:
                return value;
        }
    }
    /**
     * Replace Placeholders in Incoming Text
     * @param {string} text
     * @param {Indexable} e
     * @return {string}
     */
    static replacePlaceholdersInText(text, e) {
        let match = null;
        while (match = text.match(ViewHelper.regexTemplate)) {
            let value = ObjectHelper.get(e, match[1]);
            value = value || '';
            value = this.pipeUse(match[2], value);
    ...
    /**
    * @private
     * Replace Placeholders in Incoming InnerHTML
     * @param {HTMLElement} template
     * @param {Indexable} e
     */
    static replacePlaceholdersInnerHTML(template, e) {
        let match = null;
        ...
        while (match = text.match(ViewHelper.regexTemplate)) {
            ...
            value = value || '';
            value = this.pipeUse(match[2], value);
            template.innerHTML = text.replace(match[0], value);
            text = template.innerHTML;
        }
    }
    /**
    * @private
     * Replace Placeholders in Incoming NodeText
     * @param {ChildNode} template
     * @param {Indexable} e
     */
    static replacePlaceholdersInNodeText(template, e) {
        let match = null;
        let text = template.textContent || '';
        while (match = text.match(ViewHelper.regexTemplate)) {
            let value = ObjectHelper.get(e, match[1]);
            value = value || '';
            template.textContent = text.replace(match[0], value);
...
class VetproviehBinding {
    ...
    _alreadyBound(element, attribute) {
        return this._bindings
            .findIndex((b) => {
            return this._compareElement(element, b, attribute);
        }) >= 0;
    }
    /**
     * @private
     * @param {HTMLElement} a
     * @param {{ element: HTMLElement, attribute: string }} b
     * @param {String} attribute
     * @return {boolean}
     */
    _compareElement(a, b, attribute) {
        return b.element.tagName === a.tagName && b.attribute === attribute;
    }
    /**
   ...
   * @param {HTMLElement|any} target (Target - HTMLElement)
   * @param {any} data (DataObject)
   * @param {String} prefix
   * @param {any} listener
   */
    static bindFormElements(target, data, prefix = '', listener = () => null) {
        Object.keys(data).forEach((key) => {
            if (ObjectHelper.isObject(data[key]) && !Array.isArray(data[key])) {
                this.bindFormElements(target, data[key], (prefix != '' ? prefix : '') + key + '.', listener);
            }
            else {
                ...
                if (element) {
                    if (target)
                        this.setUniqueIdentifier(target);
                    const binding = VetproviehBinding
                        .bindElement(prefix, key, target['uniqueId'] || target.id, data);
     ...
     * @param {HTMLElement} element
     */
    static setUniqueIdentifier(element) {
        if (!element.id) {
            element.id = 'id_' + VetproviehBinding.uniqueIdentifier++;
        }
    }
    /**
     * Binding an Element
    ...
    static bindElement(prefix, key, targetId, data) {
        const bKey = targetId + '-' + prefix + key;
        let binding = VetproviehBinding
            .currentBindings[bKey];
        if (!binding) {
            binding = new VetproviehBinding(data, key);
            VetproviehBinding.currentBindings[bKey] = binding;
        }
        else {
...
VetproviehBinding.uniqueIdentifier = 0;
...
class VetproviehElement extends HTMLElement {
    ...
    static get template() {
        return `<link 
    href="/node_modules/bulma/css/bulma.min.css" crossorigin="anonymous" 
    rel="stylesheet preload prefetch" type="text/css" as="style">
    <link href="/node_modules/fontawesome/css/all.min.css" 
    crossorigin="anonymous" rel="stylesheet preload prefetch" 
    type="text/css" as="style">
    <link crossorigin="anonymous" rel="preload" 
    href="/node_modules/fontawesome/webfonts/fa-solid-900.woff2" 
...
class VetproviehBasicRepeat extends VetproviehElement {
    ...
    /**
     * Get Function for afterClearAndRender
     * @return {Function | undefined}
     */
    get afterClearAndRender() {
        return this._afterClearAndRender;
    }
    /**
     * Set Function for afterClearAndRender
     * @param {Function | undefined} func
     */
    set afterClearAndRender(func) {
        if (func !== this._afterClearAndRender) {
            this._afterClearAndRender = func;
        }
    }
    /**
    * Get OrderBy
    ...
    */
    ...
    delete(index) {
        if (this.objects.length > index) {
            ...
            if (this.repository) {
                this.repository.destroy(obj.id).then((result) => {
                    if (result) {
    ...
    */
    ...
    clearAndRender() {
        ...
        this._sortObjects();
        this.renderList();
        if (this._afterClearAndRender !== undefined)
            this._afterClearAndRender();
    }
    /**
     * Sorting Objects
     */
 ...
 * @param {IWebComponentArgs} webComponentArgs
 * @return {any}
 */
function WebComponent(webComponentArgs) {
    /**
       ...
       * @param {string | undefined} extendElement
       */
...
let VetproviehRepeat = 
/**
 * Repeats Template Element. Amount is set by the amount of objects
 * inside
 */
class VetproviehRepeat extends VetproviehBasicRepeat {
};
 ...
 */
class VetproviehTable extends VetproviehBasicRepeat {
    /**
     * ConnectedCallback
     */
    connectedCallback() {
    ...
    getElementByIndex(index) {
        ...
        return itemBox === null || itemBox === void 0 ? void 0 : itemBox.querySelector(selector);
    }
    /**
     * Inserts Element to List
     * @param {any} dataItem
    ...
    _attachToList(dataItem, index = 0) {
        if (this.shadowRoot) {
            ...
            const newListItem = this._generateListItem(item);
            ...
            if (deleteBtn) {
                deleteBtn['data-id'] = index;
                deleteBtn.addEventListener('click', (event) => {
                    this.delete(event.target['data-id']);
                });
            }
     ...
     */
    deleteButtonFromItem(listItem) {
        const tagSearch = 'button[data-action="delete"]';
        return listItem.querySelector(tagSearch);
    }
    /**
...
VetproviehTable = __decorate([
  ...
  <style> .dragable { cursor: move} .small-td { width: 50px; }</style>
...
class VetproviehSelect extends VetproviehElement {
     ...
     */
    get list() {
        return this.shadowRoot.getElementById('farmerList');
    }
    /**
